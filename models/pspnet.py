...
from keras.layers import merge, multiply, add, concatenate, BatchNormalization, Cropping2D, ZeroPadding2D
...
def pyramid_pooling_module(
    ...
    for level in levels:
        #print(level)
        interp_out = interp_block(
                x,
                num_filters=num_filters,
                level=level,
                input_shape=input_shape,
                output_stride=output_stride)
        interp_out = ZeroPadding2D(padding = ((0, 1), (0, 0)))(interp_out)
        #print(interp_out.shape)
        pyramid_pooling_blocks.append(interp_out)
...
def PSPNet50(
    ...
    out = Conv2D(n_labels, (1, 1), activation=output_mode)(out)
